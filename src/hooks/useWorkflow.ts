...
function collectWorkflowLoadErrors(
  ...
  for (const node of workflow.nodes) {
    ...
    for (const name of orderedInputs) {
      ...
      const hasMatch = resolved !== undefined;
  ...
  persist(
    (set, get) => {
      ...
      applyLoraCodeUpdate: (payload) => {
        ...
        const numericId = typeof rawNodeId === 'string'
          ? Number(rawNodeId)
          : (typeof rawNodeId === 'number' ? rawNodeId : null);
        ...
        targets.forEach((node) => {
          ...
          const currentText = textIndex !== null
            ? String(widgetValues[textIndex] ?? '')
            : '';
          const nextText = mode === 'replace'
            ? loraCode
            : (currentText.trim() ? `${currentText.trim()} ${loraCode}` : loraCode);
          const currentList = listIndex !== null && Array.isArray(widgetValues[listIndex])
            ? widgetValues[listIndex]
            : [];
          const mergedList = mergeLoras(nextText, currentList as Array<{ name: string; strength: number | string }>);
          ...
          if (Object.keys(updates).length > 0) {
            get().updateNodeWidgets(node.id, updates);
            get().syncTriggerWordsForNode(node.id);
          }
        });
      },

      applyTriggerWordUpdate: (payload) => {
        const { workflow, nodeTypes } = get();
        ...
        targets.forEach((node) => {
          ...
          const groupModeIndex = getWidgetIndexForInput(workflow, nodeTypes, node, 'group_mode');
          const defaultActiveIndex = getWidgetIndexForInput(workflow, nodeTypes, node, 'default_active');
          const allowStrengthIndex = getWidgetIndexForInput(workflow, nodeTypes, node, 'allow_strength_adjustment');
          ...
          const defaultActive = defaultActiveIndex !== null
            ...
            : true;
          const allowStrength = allowStrengthIndex !== null
          ...
          const currentList = extractTriggerWordList(node.widgets_values[listIndex]) ??
      ...
      applyWidgetUpdate: (payload) => {
        ...
        targets.forEach((node) => {
          ...
          if (nodeTypes) {
            const typeDef = nodeTypes[node.type];
            const inputDef = typeDef?.input?.required?.[widgetName] || typeDef?.input?.optional?.[widgetName];
          ...
          if (!Array.isArray(node.widgets_values)) {
            get().updateNodeWidget(node.id, 0, nextValue, widgetName);
            return;
          }
          ...
          if (isLoraManagerNode && widgetName === 'loras') {
            ...
            const listValue = extractLoraList(nextValue) ?? (Array.isArray(nextValue) ? nextValue : null);
          ...
          if (index !== null) {
            get().updateNodeWidget(node.id, index, nextValue, widgetName);
            if (isLoraManagerNode && (widgetName === 'cycler_config' || widgetName === 'randomizer_config')) {
              get().syncTriggerWordsForNode(node.id);
            }
          }
        });
      ...
      syncTriggerWordsForNode: (nodeId) => {
        const { workflow, nodeTypes } = get();
        if (!workflow) return;
        ...
        const updateConnectedTriggerWords = (
          ...
          void api.requestTriggerWords(Array.from(loraNames), references)
            .catch((error) => {
        ...
        const getActiveLorasFromNode = (node: WorkflowNode): Set<string> => {
          ...
          if (isLoraCyclerNodeType(node.type)) {
            ...
            if (nodeTypes) {
              const configIndex = getWidgetIndexForInput(workflow, nodeTypes, node, 'cycler_config');
              if (configIndex !== null) {
                config = widgetValues[configIndex];
              }
            }
            if (!config) {
              config = widgetValues.find((value) =>
                isRecord(value) && (
            ...
            if (isRecord(config)) {
              const name = typeof config.current_lora_filename === 'string'
          ...
          const list = extractLoraList(rawList) ??
            (Array.isArray(rawList) ? rawList as Array<{ name?: string; active?: boolean }> : null);
        ...
        const getConnectedInputStackers = (node: WorkflowNode): WorkflowNode[] => {
          ...
          for (const input of node.inputs ?? []) {
            ...
            if (sourceNode && isLoraChainProviderNodeType(sourceNode.type)) {
      ...
      registerLoraManagerNodes: async () => {
        const { workflow, nodeTypes } = get();
        if (!workflow) return;
        ...
        workflow.nodes.forEach((node) => {
          ...
          const title = typeof directTitle === 'string' && directTitle.trim()
            ? directTitle.trim()
      ...
      toggleBypass: (nodeId) => {
        ...
        set({ workflow: { ...workflow, nodes: newNodes } });
        get().syncTriggerWordsForNode(nodeId);
      },
