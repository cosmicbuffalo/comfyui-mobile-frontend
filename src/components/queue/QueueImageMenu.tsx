...
export function QueueImageMenu({
  ...
  const queueItemHideImages = useWorkflowStore((s) => s.queueItemHideImages[promptId]);
  const toggleQueueItemHideImages = useWorkflowStore((s) => s.toggleQueueItemHideImages);
  ...
  return createPortal(
    ...
    >
      ...
      {menuState.hasVideoOutputs && menuState.promptId && (
        <button
          className="flex items-center gap-2 w-full text-left px-3 py-2 text-sm hover:bg-gray-50"
          onClick={() => {
