...
interface QueueCardProps {
  ...
  onOpenMenu: (payload: { top: number; right: number; imageSrc: string; workflow?: Workflow; promptId?: string; hasVideoOutputs?: boolean }) => void;
...
export function QueueCard({
  ...
  const queueItemExpanded = useWorkflowStore((s) => s.queueItemExpanded[item.id]);
  const setQueueItemExpanded = useWorkflowStore((s) => s.setQueueItemExpanded);
  const queueItemHideImages = useWorkflowStore((s) => s.queueItemHideImages[item.id]);
                      ...
                      {isVideoFilename(img.filename) ? (
                        <>
                          <video
                            ...
                            onClick={() => {
                              const index = queueViewerImages.findIndex((entry) => entry.src === src);
                              onImageClick?.(queueViewerImages, index >= 0 ? index : i, isTopDoneItem);
                            }}
                          />
