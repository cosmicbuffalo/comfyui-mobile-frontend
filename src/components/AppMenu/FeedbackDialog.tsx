...
function errorMessageFor(error: string, status?: number): string {
  switch (error) {
    case 'rate_limited':
      return t('Too many submissions in a short window. Please try again in a minute.');
    case 'invalid_fields':
      return t('The submission was rejected as invalid. Please review your inputs, or');
    case 'github_create_failed':
      return t("We couldn't create the issue right now (the feedback service is having trouble). Please");
    case 'timeout':
      return t('The request timed out. The feedback service may be slow — please try again, or');
    case 'network_error':
      return t("Couldn't reach the feedback service. Check your connection, or");
    default:
      return status
        ? t('Submission failed (HTTP {status}). Please', { status })
        : t('Submission failed. Please');
  }
}
