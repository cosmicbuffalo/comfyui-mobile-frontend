...
function getTypeClass(type: string): string {
  const normalizedType = type.split(',')[0].trim(); // Handle multi-types like "FLOAT,INT"
...
export const ConnectionButton = memo(function ConnectionButton({
  ...
  const { effectiveNodes, directNodes, bypassedTargets } = useMemo(() => {
    ...
    if (!hideBypassedNodes && !hideStaticNodes) {
    ...
    const hasEditableInputs = (node: Workflow['nodes'][number]) => {
  ...
  }, [connectedNodes, hideBypassedNodes, hideStaticNodes, nodeTypes, workflow, direction, slot.type, manuallyHiddenNodes]);
