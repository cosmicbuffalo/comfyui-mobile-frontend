...
export const NodeCard = memo(function NodeCard({ node, isExecuting, isConnectionHighlighted = false, onImageClick }: NodeCardProps) {
  ...
  const setSeedMode = useWorkflowStore((s) => s.setSeedMode);
  // Only subscribe to whether THIS node has a bookmarked widget (reduces re-renders)
  const bookmarkedWidgetForThisNode = useWorkflowStore((s) =>
    s.bookmarkedWidget?.nodeId === node.id ? s.bookmarkedWidget : null
  );
  const setBookmarkedWidget = useWorkflowStore((s) => s.setBookmarkedWidget);
  const nodeImages = useWorkflowStore((s) => s.nodeOutputs[String(node.id)]);
  const nodeErrors = useWorkflowStore((s) => s.nodeErrors[String(node.id)]);
  ...
  const displayName: string = typeDef?.display_name || node.type;
  ...
  // Collect all bookmarkable widgets for the bookmark submenu
  const bookmarkableWidgets = useMemo(() => {
  ...
  // const hasEditableParams = (node.widgets_values?.length ?? 0) > 0 ||
  //   node.inputs?.some((input) => Boolean(input.widget)) ||
  //   visibleWidgets.length > 0 ||
  //   visibleInputWidgets.length > 0;
  ...
  const connectionInputs = useMemo(() =>
    node.inputs.filter((input) => !isWidgetInput(input)),
  ...
  // Helper to check if a widget is bookmarked
  const isWidgetBookmarked = (widgetIndex: number) => {
    return bookmarkedWidgetForThisNode?.widgetIndex === widgetIndex;
  };

  // Helper to toggle bookmark for a widget
  const toggleWidgetBookmark = (widgetIndex: number, widgetName: string, widgetType: string, options?: Record<string, unknown> | unknown[]) => {
    if (isWidgetBookmarked(widgetIndex)) {
  ...
  const singleBookmarkableWidget = bookmarkableWidgets.length === 1 ? bookmarkableWidgets[0] : null;
  const isSingleWidgetBookmarked = singleBookmarkableWidget
    ? isWidgetBookmarked(singleBookmarkableWidget.widgetIndex)
    : false;
  const hasNodeBookmark = Boolean(bookmarkedWidgetForThisNode);
  ...
  const previewList = effectivePreviewImage
    ? [{ src: getImageUrl(effectivePreviewImage.filename, effectivePreviewImage.subfolder, effectivePreviewImage.type), alt: displayName }]
    : [];
  // const nodeIdLabel = String(node.id);
  ...
  return (
    <div className="relative">
      ...
      <NodeCardHeader
        ...
        rightSlot={(
          <NodeCardMenu
            nodeId={node.id}
            isBypassed={isBypassed}
            bookmarkableWidgets={bookmarkableWidgets}
            singleBookmarkableWidget={singleBookmarkableWidget}
            isSingleWidgetBookmarked={isSingleWidgetBookmarked}
            hasNodeBookmark={hasNodeBookmark}
            toggleWidgetBookmark={toggleWidgetBookmark}
            setBookmarkedWidget={setBookmarkedWidget}
      ...
      {!isCollapsed && (
        <div className={isBypassed ? 'opacity-60 grayscale' : ''}>
          ...
          <NodeCardParameters
            ...
            isWidgetBookmarked={isWidgetBookmarked}
            toggleWidgetBookmark={toggleWidgetBookmark}
          />
          {noteText && (
            <NodeCardNote
